...
export default function FloorGrid({ grid, onSelectCell, playerTerritorySize, justConqueredCellId, language, t }: FloorGridProps) {
  ...
  // KROK 1 OPTYMALIZACJI: Szybki Set struktur row-col dla stałego czasu dostępu O(1)
  ...
  // KROK 2 OPTYMALIZACJI: Inteligentne pozycjonowanie Badge'a przy użyciu odległości Manhattan
  ...
  return (
    <div className="w-full font-sans text-cocoa">
      ...
      <div className="mb-3 flex items-center justify-center gap-4 text-[10px] font-black text-[#5A3A2A] bg-white border-2 border-[#5A3A2A] py-1.5 px-4 rounded-[16px] leading-none w-full select-none shadow-[0_3px_0_#5A3A2A] antialiased">
        ...
        <div className="flex items-center gap-1.5">
          <span className="w-3 h-3 rounded-full bg-[#7A604A] border-2 border-[#5A3A2A] flex-shrink-0 flex items-center justify-center text-[8px] font-black text-white shadow-[0_1px_0_#5A3A2A] select-none leading-none">✕</span>
      ...
      <div className="relative overflow-hidden rounded-[24px] bg-[#FFF4DF] p-2 sm:p-3.5 shadow-md border-2 border-[#5A3A2A]">
        ...
        >
          {gridItems.map((item) => {
            ...
            // OPTYMALIZACJA WYDAJNOŚCI: Wykorzystanie Setu zamiast przeszukiwania gridu metodą .some()
            ...
            return (
              <button
                ...
                className={`relative flex flex-col items-center justify-between p-1 pt-1.5 pb-1 transition-all outline-none ${
              ...
              >
                {!item.isPlayer && (
                  <span className={`text-base sm:text-xl z-10 select-none transition-all leading-none ${(!item.isPlayer && !item.isAdjacent) ? "grayscale contrast-75 brightness-95 opacity-50" : ""}`}>
                    {owner.avatar}
                  </span>
                )}
                ...
                {item.isAdjacent && (
                  <div className="absolute inset-0 bg-lemon-yellow/10 opacity-0 hover:opacity-100 flex items-center justify-center transition-all bg-[#FFF4DF]/20 rounded-xl sm:rounded-2xl z-20">
          ...
          {playerBadgeStyle && (
            <div 
              ...
              className="bg-white border-2 border-[#24456B] shadow-[0_4px_0_#24456B] px-3.5 py-1.5 rounded-[20px] flex items-center justify-center gap-1.5 pointer-events-none z-30 animate-pulse"
            >
